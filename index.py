...
async def RunMain(app_id: str, secret: str, ws_key: str, bot_name: str, ws_url: str, sandbox: bool, webhook: bool):
    """执行机器人的主运行流程。"""
    try:
        if webhook:
...
def InteractiveSetup(config_manager: ConfigManager):
    """执行首次启动时的交互式配置向导。"""
    print("\n== 首次配置向导 ==")
...
if __name__ == '__main__':
    ...
    if not config_manager.Exists():
        InteractiveSetup(config_manager)
